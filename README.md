...
- **Offre gratuite / Étudiant+** : la version gratuite est limitée à 15 candidatures suivies (compteur cumulé, insensible aux suppressions) et un quota IA quotidien restreint ; l'offre Étudiant+ (paiement Stripe) lève ces limites. Tant que Stripe n'est pas configuré (`STRIPE_PRICE_ID` absent), le bouton "Passer à Étudiant+" est automatiquement remplacé par une inscription à une liste d'attente.
