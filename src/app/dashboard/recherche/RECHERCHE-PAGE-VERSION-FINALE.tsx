...
export default function RecherchePage() {
  ...
  async function handleRecherche(e: any) {
    e.preventDefault();
    setLoading(true);
    setErreur(null);
    setOffres(null);
    ...
    const secteur = SECTEURS.find(function (s: any) {
      ...
      <form onSubmit={handleRecherche} className="bg-white border border-gray-200 rounded-lg p-5 mb-8 space-y-4">
        <div>
          ...
          <input
            type="text"
            value={codePostal}
            onChange={function (e: any) { setCodePostal(e.target.value); }}
        ...
        <div>
          ...
          <select
            value={secteurChoisi}
            onChange={function (e: any) { setSecteurChoisi(e.target.value); }}
            className="w-full border border-gray-300 rounded-md px-3 py-2"
          >
            {SECTEURS.map(function (s: any) {
              return (
        ...
        <div>
          ...
          <input
            ...
            step={5}
            value={rayon}
            onChange={function (e: any) { setRayon(Number(e.target.value)); }}
            className="w-full"
          />
        </div>
      ...
      {entreprisesPotentielles && entreprisesPotentielles.length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {entreprisesPotentielles.map(function (ent: any, i: any) {
              return (
